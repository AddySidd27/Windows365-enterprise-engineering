...
**Evidence status:** Earlier Business lab work covered Intune enrollment, Edge and desktop settings, Company Portal, and creation of an update ring and compliance policy. No screenshots or logs from that lab are published here, so those observations cannot yet be independently verified. A [read-only Enterprise portal check](evidence/UC-02-2026-09-24-portal-check.md) now confirms one existing Provisioned Cloud PC, its policy and Intune record. It does not show a newly run provisioning workflow or user/device-side validation. Azure network connection, resize, move, restore, and reprovision remain documented procedures without completed tests. See the [evidence index](evidence/README.md) before treating any procedure as a tested result.
