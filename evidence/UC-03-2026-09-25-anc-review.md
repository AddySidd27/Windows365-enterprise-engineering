...
- The Azure lab resource group contained the East US virtual network, a NAT gateway and a public IP, plus an earlier Central US virtual network.
...
**Blocked before submission.** No connection was created in this check. There is no ANC health result, no test Cloud PC provisioned on this network, and no device-side route or DNS proof. The existing Microsoft-hosted Cloud PC was not moved. The NAT gateway and public IP were still present at this observation; their cleanup must be verified separately. This record is a portal observation, not a passed end-to-end lab.
